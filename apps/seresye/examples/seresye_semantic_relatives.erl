...
%%%-------------------------------------------------------------------
%%%
-module(seresye_semantic_relatives).
...
start() ->
  ...
  case semantic:start() of
    ...
    {ok, []} ->
      ...
      seresye_srv:start(seresye_semantic_relatives),
      % Add rules from the current module to the seresye server.
      seresye_srv:add_rules(seresye_semantic_relatives, ?MODULE),
      ...
      lists:foreach(fun(Tuple) ->  seresye_srv:assert(seresye_semantic_relatives, Tuple) end, ListOfTuples),
      ok;
