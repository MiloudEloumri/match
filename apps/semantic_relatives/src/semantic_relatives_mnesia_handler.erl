...
-spec get_all_records() -> list(#semantic_relatives_kb{}).
...
-spec query_kb(atom(), {any(), any(), fun() | any()}) -> list(any()).
...
query_kb(Table, {Predicate, Object, SubjectFun}) when is_function(SubjectFun) ->
    ...
    {atomic, Records} = mnesia:transaction(Fun),
    FilteredRecords = lists:filter(fun({_, _, _, Subject}) -> SubjectFun(Subject) end, Records),
...
query_kb(Table, {Predicate, Object, Subject}) ->
    MatchSpec = [{{Table, Predicate, Object, Subject}, [], 
                  ['$_']}],
    Fun = fun() -> mnesia:select(Table, MatchSpec) end,
